...
from graphgps.layer.SGGN_layer import SGGN, SGGN_without_sort, SGGN_without_sort_withoutC
...
class GPSLayer(nn.Module):
    ...
    def __init__(self, dim_h,
                 local_gnn_type, global_model_type, num_heads, act='relu', max_degree=1,
        ...
        elif local_gnn_type == "SGGN":
            self.local_model = SGGN(dim_h, max_degree=max_degree )
        elif local_gnn_type == "SGGN_without_sort":
            self.local_model = SGGN_without_sort(dim_h, max_degree=max_degree )
        elif local_gnn_type == "SGGN_without_sort-C":
            self.local_model = SGGN_without_sort_withoutC(dim_h, max_degree=max_degree )
        else:
    ...
    def forward(self, batch):
        ...
        if self.local_model is not None:
            ...
            if self.local_gnn_type == 'CustomGatedGCN':
                ...
                if self.local_gnn_type == "SGGN" or self.local_gnn_type == "SGGN_without_sort" or self.local_gnn_type == "SGGN_without_sort-C":
                    h_local = self.local_model(batch).x
