...
"""
from nets.random_graph_regression.load_net import gnn_model 
...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs):
    ...
    if "RWSE" in net_params:
        if net_params['RWSE']:
            st = time.time()
            print("[!] Adding random walk positional encoding.")
            dataset._init_positional_encodings(net_params['pos_enc_dim'])
            print('Time RWSE:',time.time()-st)
    try:
    ...
    from train.train_random_graph_regression import  evaluate_network



    '''
    import train_epoch when comparing GPU Memory
    import train_epoch_flops when comparing FLOPs
    '''
    # from train.train_random_graph_regression import train_epoch_flops as train_epoch
    from train.train_random_graph_regression import train_epoch as train_epoch
    ...
    try:
        with tqdm(range(params['epochs'])) as t:
            for epoch in t:
                ...
                print(int(torch.cuda.max_memory_allocated()/1024/1024))
    ...
    Convergence Time (Epochs): {:.4f}\nTotal Time Taken: {:.4f} hrs\nAverage Time Per Epoch: {:.4f} s\n\n\n"""\
          .format("random", MODEL_NAME, params, net_params, model, net_params['total_param'],
...
def main():    
    ...
    dataset = LoadData("random", num_nodes)
    # print(dataset)
    ...
    # print(type(dataset.train.graph_lists[0].adjacency_matrix()))

    # D = torch.cat([g.in_degrees() for g in
