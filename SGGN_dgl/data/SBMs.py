...
def count_frequency(lst):
    ...
    # 将频次写入文件
    with open("frequency_count.txt", "w") as file:
        for number, count in frequency_counter.items():
            file.write(f"Number: {number}, Count: {count}\n")
...
class SBMsDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, name):
        ...
        for tu in (self.train + self.val + self.test):
            ...
            all_degrees += in_degrees.tolist()
            temp,_ = in_degrees.max(dim = 0)
            max_degrees.append(int(temp))
            if temp > max_degree:
                max_degree = temp
        print(int(max_degree))  
        # print( sorted(max_degrees) )
        count_frequency(sorted(all_degrees))
