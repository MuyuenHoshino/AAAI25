...
class SuperPixDGL(torch.utils.data.Dataset):
    ...
    def _prepare(self):
        ...
        # for index, sample in tqdm(enumerate(self.sp_data)):
...
class DGLFormDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # return tuple(li[index] for li in tqdm(self.lists))
...
class SuperPixDatasetDGL(torch.utils.data.Dataset):
    def __init__(self, name, num_val=5000):
        ...
        use_coord = True
        self.test = SuperPixDGL("/home/guojiahe/benchmarking-gnns/data/superpixels/superpixels", dataset=self.name, split='test', 
                            use_mean_px=use_mean_px, 
                            use_coord=use_coord)

        self.train_ = SuperPixDGL("/home/guojiahe/benchmarking-gnns/data/superpixels/superpixels", dataset=self.name, split='train', 
                             use_mean_px=use_mean_px, 
                             use_coord=use_coord)
