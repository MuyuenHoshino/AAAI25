...
class TUsDataset(torch.utils.data.Dataset):
    def __init__(self, name):
        ...
        num = 0
        max_degree = 0
        for tud in (self.train + self.val + self.test):
            for tu in tud:
                num += 1
                g, l = tu
        ...
        is_directed = g.is_multigraph and g.is_directed()
        # print(f"图是否是有向的: {is_directed}")
        print("avg nodes", total_nodes/num)
        print("avg edges", total_edges/num)
        print("#################################")
